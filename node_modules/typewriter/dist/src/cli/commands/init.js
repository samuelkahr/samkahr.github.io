...
exports.Init = props => {
    ...
    const [sdk, setSDK] = react_1.useState(options_1.SDK.WEB);
    const [language, setLanguage] = react_1.useState(options_1.Language.JAVASCRIPT);
    ...
    return (react_1.default.createElement(ink_1.Box, { minHeight: 20, marginLeft: 2, marginRight: 2, marginTop: 1, marginBottom: 1, flexDirection: "column" },
        ...
        step === 0 && react_1.default.createElement(ConfirmationPrompt, { onSubmit: onNext }),
        step === 1 && react_1.default.createElement(SDKPrompt, { step: step, sdk: sdk, onSubmit: withNextStep(setSDK) }),
        step === 2 && (react_1.default.createElement(LanguagePrompt, { step: step, sdk: sdk, language: language, onSubmit: withNextStep(setLanguage) })),
        step === 3 && (react_1.default.createElement(APITokenPrompt, { step: step, config: config, onSubmit: withNextStep(setTokenMetadata) })),
        step === 4 && (react_1.default.createElement(TrackingPlanPrompt, { step: step, token: tokenMetadata.token, trackingPlan: trackingPlan, onSubmit: withNextStep(setTrackingPlan) })),
        step === 5 && react_1.default.createElement(PathPrompt, { step: step, path: path, onSubmit: withNextStep(setPath) }),
        step === 6 && (react_1.default.createElement(SummaryPrompt, { step: step, sdk: sdk, language: language, path: path, token: tokenMetadata.token, workspace: tokenMetadata.workspace, trackingPlan: trackingPlan, onConfirm: onNext, onRestart: onRestart })),
        step === 7 && react_1.default.createElement(build_1.Build, Object.assign({}, props, { production: false, update: true }))));
};
const Header = () => {
...
const ConfirmationPrompt = ({ text, onSubmit }) => {
    const items = [{ label: 'Ok!', value: 'ok' }];
    const tips = ['Hit return to continue.'];
    return (react_1.default.createElement(Step, { name: text || 'Ready', question: true, tips: tips },
...
const APITokenPrompt = ({ step, config, onSubmit }) => {
    const [token, setToken] = react_1.useState('');
    const [canBeSet, setCanBeSet] = react_1.useState(true);
    const [workspace, setWorkspace] = react_1.useState();
    const [isLoading, setIsLoading] = react_1.useState(true);
    const [isInvalid, setIsInvalid] = react_1.useState(false);
    const [foundCachedToken, setFoundCachedToken] = react_1.useState(false);
    react_1.useEffect(() => {
        ;
        (() => __awaiter(this, void 0, void 0, function* () {
            const tokens = yield config_1.listTokens(config);
            const method = yield config_1.getTokenMethod(config);
    ...
    const onConfirm = () => __awaiter(this, void 0, void 0, function* () {
        ...
        setIsLoading(true);
        const result = yield api_1.validateToken(token);
        if (result.isValid) {
            yield config_1.storeToken(token);
    ...
    return (react_1.default.createElement(Step, { name: "Enter a Segment API token", step: step, isLoading: isLoading, tips: tips },
        !canBeSet && react_1.default.createElement(ink_select_input_1.default, { items: [{ label: 'Ok!', value: 'ok' }], onSelect: onConfirm }),
        canBeSet && foundCachedToken && (react_1.default.createElement(ink_select_input_1.default, { items: [
                { label: 'Use this token', value: 'yes' },
                { label: 'No, provide a different token.', value: 'no' },
            ], onSelect: onConfirmCachedToken })),
        canBeSet && !foundCachedToken && (react_1.default.createElement(ink_1.Box, { flexDirection: "column" },
            react_1.default.createElement(ink_1.Box, null,
                react_1.default.createElement(ink_1.Text, null, figures_1.default.pointer),
                ' ',
                react_1.default.createElement(ink_text_input_1.default, { value: token, 
                    // See: https://github.com/vadimdemedes/ink-text-input/issues/41
                    showCursor: true, onChange: setToken, onSubmit: onConfirm, mask: "*" })),
            isInvalid && (react_1.default.createElement(ink_1.Box, { textWrap: "wrap", marginLeft: 2 },
...
const TrackingPlanPrompt = ({ step, token, trackingPlan, onSubmit, }) => {
    ...
    return (react_1.default.createElement(Step, { name: "Tracking Plan", tips: tips, step: step, isLoading: isLoading },
        react_1.default.createElement(ink_select_input_1.default, { items: choices, onSelect: onSelect, initialIndex: initialIndex, limit: 10 })));
};
...
const SummaryPrompt = ({ step, sdk, language, path, token, workspace, trackingPlan, onConfirm, onRestart, }) => {
    ...
    const summaryRows = [
        ...
        {
            label: 'Tracking Plan',
            value: (react_1.default.createElement(ink_link_1.default, { url: api_1.parseTrackingPlanName(trackingPlan.name).url }, trackingPlan.display_name)),
        },
    ];
...
const Step = ({ step, name, isLoading = false, question = false, description, tips, children, }) => {
    return (react_1.default.createElement(ink_1.Box, { flexDirection: "column" },
